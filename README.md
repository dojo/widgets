...
Icon fonts are generated using [IcoMoon](https://icomoon.io/app). If a new icon is required, it is possible to upload the current `dojoSelect.json` from `src/theme/fonts` and then add new icons by selecting from the Font Awesome library. After selecting the new icons from the library, merge them down into the current icon set, then delete the rest of the Font Awsome icons that were added by IcoMoon. After this you can and export and download them as a zip. Once downloaded you will also need to unzip them and replace the font files (svg,woff, ttf) in `src/theme/fonts`. After this download the new selection JSON file from the `projects` page of IcoMoon and replace the current `dojoSelection.json` file.
...
4. All widgets must have a `README.md` file in their root directory.

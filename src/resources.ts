...
import { create, diffProperty, invalidator, destroy } from '@dojo/framework/core/vdom';
...
export interface ResourcePut<RESOURCE> {
	(
		readResponse: ResourceReadResponse<RESOURCE>,
		readRequest: ResourceReadRequest<RESOURCE>
	): void;
	(
		findResponse: ResourceFindResponse<RESOURCE> | undefined,
		findRequest: ResourceFindRequest<RESOURCE>
	): void;
}
...
export interface ResourceRead<RESOURCE> {
	(request: ResourceReadRequest<RESOURCE>, controls: ResourceControls<RESOURCE>): void | Promise<
		void
	>;
}
export interface ResourceFind<RESOURCE> {
	(options: ResourceFindRequest<RESOURCE>, controls: ResourceControls<RESOURCE>): void | Promise<
		void
	>;
}
...
export function createMemoryResourceTemplate<RESOURCE = void>(): ResourceTemplateWithInit<
	...
	{ data: RESOURCE[] }
> & { type: 'init' } {
	return { ...memoryTemplate } as ResourceTemplateWithInit<RESOURCE, { data: RESOURCE[] }> & {
		type: 'init';
	};
}
...
export function defaultFind(
	request: ResourceFindRequest<any>,
	{ put, get }: ResourceControls<any>
) {
...
function getDataKey({
	query = {}
}: Partial<ResourceOptions<any>> | Partial<ResourceReadRequest<any>> = {}): string {
...
function diffInitOptions(current: any, next: any) {
	...
	return [...keys].some((initKey) => auto(current[initKey], next[initKey]).changed);
}
...
const idToResourceMap = new Map<
	string,
	Set<{ resource: Resource<any>; type: 'owner' | 'subscribed' }>
>();
...
function createOptionsWrapper(): Options<any> {
	...
	function setOptions(newOptions?: Partial<ResourceOptions<any>>): ResourceOptions<any> {
		if (newOptions) {
			...
			const changed = auto(options, calculatedOptions);
			if (changed) {
...
function getResource(
	...
	const templateId =
		(isResource(templateOrWrapper) ? templateOrWrapper.id : init && `${id}/${init.id}`) ||
		'global';
	...
	const registeredResources =
		idToResourceMap.get(id) ||
		new Set<{ resource: Resource<any>; type: 'owner' | 'subscribed' }>();
	if (!resource) {
...
export function createResourceMiddleware<MIDDLEWARE = void>() {
	...
	return factory(
		({
			id: middlewareId,
			middleware: { diffProperty, invalidator, destroy }
			...
			diffProperty(
				...
				({ resource: current }, { resource: next }) => {
					if (current && next) {
						...
						if (nextInitOptions) {
							const changed = diffInitOptions(
								currentInitOptions || {},
								nextInitOptions
							);
							if (changed) {
								const resourceMap = templateToResourceMap.get(
					...
					if (next) {
						...
						if (nextOptions) {
							const options: any = (options?: Partial<ResourceOptions<any>>) => {
								const invalidatorSet =
									optionInvalidatorMap.get(nextOptions.options) || new Set();
								invalidatorSet.add(invalidator);
			...
			middleware.createOptions = (key: string): Options<any> => {
				...
				function setOptions(options?: Partial<ResourceOptions<any>>) {
					const invalidatorSet =
						optionInvalidatorMap.get(optionsWrapper.options) || new Set();
					invalidatorSet.add(invalidator);

...
function generateNodeOptions(value: string, hasChildren = false) {
	return {
		id: value,
		parent: '',
		value: value,
...
describe('Tree', () => {
	...
	it('renders data', () => {
		...
		r.child(WrappedNode1, [generateNodeOptions('node-1')]);
		r.child(WrappedNode2, [generateNodeOptions('node-2')]);
	...
	describe('ExpandedNodes', () => {
		...
		it('raises events on expand/collapse on keyboard navigation with controlled nodes', () => {
			...
			r.child(WrappedNode2, [generateNodeOptions('node-2')]);
			r.expect(expandedAssertion);
			r.child(WrappedNode1, [generateNodeOptions('')]);
			r.child(WrappedNode2, [generateNodeOptions('')]);
			r.property(WrappedNode1, 'onActive');
			r.child(WrappedNode1, [generateNodeOptions('node-1')]);
			r.child(WrappedNode2, [generateNodeOptions('node-2')]);
